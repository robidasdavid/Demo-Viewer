				...
				while (!input.EOF())
				{
					...
					if (inclusionBitmask[2])
					{
						...
						f.pause = new Pause
						{
							...
							paused_timer = (float)input.ReadSystemHalf(),
							unpaused_timer = (float)input.ReadSystemHalf(),
						};
					}
					else
					{
					...
					if (inclusionBitmask[4])
					{
						...
						f.last_score = new LastScore
						{
							...
							disc_speed = (float)input.ReadSystemHalf(),
							distance_thrown = (float)input.ReadSystemHalf(),
						};
					}
					else
					{
					...
					if (inclusionBitmask[5])
					{
						...
						{
							arm_speed = (float)input.ReadSystemHalf(),
							total_speed = (float)input.ReadSystemHalf(),
							off_axis_spin_deg = (float)input.ReadSystemHalf(),
							wrist_throw_penalty = (float)input.ReadSystemHalf(),
							rot_per_sec = (float)input.ReadSystemHalf(),
							pot_speed_from_rot = (float)input.ReadSystemHalf(),
							speed_from_arm = (float)input.ReadSystemHalf(),
							speed_from_movement = (float)input.ReadSystemHalf(),
							speed_from_wrist = (float)input.ReadSystemHalf(),
							wrist_align_to_throw_deg = (float)input.ReadSystemHalf(),
							throw_align_to_movement_deg = (float)input.ReadSystemHalf(),
							off_axis_penalty = (float)input.ReadSystemHalf(),
							throw_move_penalty = (float)input.ReadSystemHalf()
						};
					}
					else
					{
					...
					if (inclusionBitmask[7])
					{
						...
						f.disc = new Disc
						{
							...
							{
								(float)input.ReadSystemHalf() + (lastFrame?.disc.velocity[0] ?? 0),
								(float)input.ReadSystemHalf() + (lastFrame?.disc.velocity[1] ?? 0),
								(float)input.ReadSystemHalf() + (lastFrame?.disc.velocity[2] ?? 0),
							}
						};
					}
					else
					...
					for (int i = 0; i < 3; i++)
					{
						...
						for (int j = 0; j < teamPlayerCount; j++)
						{
							...
							if (playerStateBitmask[5])
							{
								...
								p.packetlossratio = (float)input.ReadSystemHalf() +
