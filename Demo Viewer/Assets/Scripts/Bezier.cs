...
namespace Spark
{
	...
	public class BezierSpline
	{
		...
		public BezierSpline(IReadOnlyList<CameraTransform> keyframes)
		{
			...
			if (keyframes.Count > 2)
			{
				...
				for (int i = 1; i < keyframes.Count - 1; i++)
				{
					...
					Quaternion rotDiff = keyframes[i + 1].rotation * Quaternion.Inverse(keyframes[i - 1].rotation);
					...
					CameraTransform handle1 = new CameraTransform(
						...
						Quaternion.Lerp(keyframes[i - 1].rotation, keyframes[i].rotation, 1/divisionFactor)
						// keyframes[i].rotation - rotDiff * .25f
					);
					CameraTransform handle2 = new CameraTransform(
						keyframes[i].position + dir / divisionFactor,
						Quaternion.Lerp(keyframes[i].rotation, keyframes[i + 1].rotation, 1/divisionFactor)
						// keyframes[i].rotation + rotDiff * .25f
					);
		...
		public Vector3 GetPoint(float t)
		{
			...
			UnityEngine.Debug.LogError("Error getting point from bezier");
			return Vector3.zero;
		}
